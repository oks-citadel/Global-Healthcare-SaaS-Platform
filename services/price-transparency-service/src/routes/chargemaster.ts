...
const bulkImportSchema = z.object({
  items: z.array(createChargemasterItemSchema).min(1, 'At least one item is required'),
...
const createPayerContractSchema = z.object({
  ...
  planType: z.enum(['commercial', 'medicare_advantage', 'medicaid_mco', 'exchange', 'other']),
  planName: z.string().optional(),
  ein: z.string().optional(),
  contractType: z.enum(['fee_for_service', 'capitation', 'value_based', 'hybrid']),
...
const createPayerRateSchema = z.object({
  ...
  rateType: z.enum(['fixed', 'percentage_of_charge', 'percentage_of_medicare', 'case_rate', 'per_diem', 'capitation']),
 ...
 * @access Private
 */
router.get('/', requireUser, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    const organizationId = req.user!.organizationId!;
    const { search, cptCode, department, isShoppable, limit, offset } = req.query;
    ...
    if (department) {
      where.departmentName = { contains: department as string, mode: 'insensitive' };
    }
 ...
 * @access Private
 */
router.get('/:id', requireUser, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    const { id } = req.params;
    const organizationId = req.user!.organizationId!;
 ...
 */
router.post('/', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
 ...
 * @access Private (admin only)
 */
router.put('/:id', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    const { id } = req.params;
    const organizationId = req.user!.organizationId!;
    ...
    if (data.revenueCode !== undefined) updateData.revenueCode = data.revenueCode;
    ...
    if (data.grossCharge !== undefined) updateData.grossCharge = data.grossCharge;
    if (data.discountedCashPrice !== undefined) updateData.discountedCashPrice = data.discountedCashPrice;
    if (data.deidentifiedMinimum !== undefined) updateData.deidentifiedMinimum = data.deidentifiedMinimum;
    if (data.deidentifiedMaximum !== undefined) updateData.deidentifiedMaximum = data.deidentifiedMaximum;
    if (data.ndcCode !== undefined) updateData.ndcCode = data.ndcCode;
    if (data.drugUnitOfMeasure !== undefined) updateData.drugUnitOfMeasure = data.drugUnitOfMeasure;
    if (data.drugQuantity !== undefined) updateData.drugQuantity = data.drugQuantity;
    if (data.modifiers !== undefined) updateData.modifiers = data.modifiers;
    if (data.isShoppable !== undefined) updateData.isShoppable = data.isShoppable;
    if (data.isBundled !== undefined) updateData.isBundled = data.isBundled;
    if (data.bundleComponents !== undefined) updateData.bundleComponents = data.bundleComponents as Prisma.JsonArray;
 ...
 * @access Private (admin only)
 */
router.delete('/:id', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    const { id } = req.params;
    const organizationId = req.user!.organizationId!;
 ...
 * @access Private (admin only)
 */
router.post('/bulk-import', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    ...
    let imported = 0;
    let updated = 0;
    let errors: Array<{ code: string; error: string }> = [];
    ...
    for (const item of items) {
      try {
          ...
          if (replaceExisting) {
            await prisma.chargemasterItem.update({
              ...
              data: {
                ...
                bundleComponents: item.bundleComponents as Prisma.JsonArray || null,
        ...
        errors.push({ code: item.code, error: err instanceof Error ? err.message : 'Unknown error' });
      }
    }
 ...
 * @access Private (admin only)
 */
router.get('/contracts', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    const organizationId = req.user!.organizationId!;
    ...
    if (payerName) {
      where.payerName = { contains: payerName as string, mode: 'insensitive' };
    }
 ...
 * @access Private (admin only)
 */
router.post('/contracts', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
 ...
 * @access Private (admin only)
 */
router.post('/contracts/:contractId/rates', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    ...
    const validation = createPayerRateSchema.safeParse({ ...req.body, payerContractId: contractId });
    if (!validation.success) {
      res.status(400).json({
        error: 'Validation Error',
        details: validation.error.errors,
 ...
 * @access Private (admin only)
 */
router.get('/export', requireUser, requireAdmin, requireOrganization, async (req: UserRequest, res: Response) => {
  try {
    const organizationId = req.user!.organizationId!;
    ...
    res.setHeader('Content-Disposition', `attachment; filename="chargemaster_${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csv);
  } catch (error) {
