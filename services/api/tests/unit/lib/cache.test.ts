...
import { CacheService, CacheError, CacheKeys, CacheTTL } from '../../../src/lib/cache.js';
...
describe('CacheService', () => {
  ...
  describe('connect', () => {
    ...
    it('should throw CacheError on connection failure', async () => {
      ...
      await expect(newService.connect()).rejects.toThrow('Failed to connect to Redis');
    });
  });
  ...
  describe('get', () => {
    it('should get value from cache', async () => {
      ...
      vi.mocked(redisClient.get).mockResolvedValue(JSON.stringify(testData));
    ...
    it('should track cache hits', async () => {
      vi.mocked(redisClient.get).mockResolvedValue(JSON.stringify({ data: 'test' }));
    ...
    it('should handle JSON parse errors', async () => {
      vi.mocked(redisClient.get).mockResolvedValue('invalid-json{');
  ...
  describe('set', () => {
    ...
    it('should throw CacheError on Redis error', async () => {
      ...
      await expect(cacheService.set('test-key', { data: 'test' })).rejects.toThrow(CacheError);
    });
  });
  ...
  describe('deletePattern', () => {
    it('should delete keys matching pattern', async () => {
      vi.mocked(redisClient.keys).mockResolvedValue(['test:user:1', 'test:user:2']);
      ...
      expect(redisClient.del).toHaveBeenCalledWith(['test:user:1', 'test:user:2']);
    ...
    it('should track deletes', async () => {
      vi.mocked(redisClient.keys).mockResolvedValue(['key1', 'key2']);
      vi.mocked(redisClient.del).mockResolvedValue(2);
    ...
    it('should throw CacheError on Redis error', async () => {
      ...
      await expect(cacheService.deletePattern('test:*')).rejects.toThrow(CacheError);
    });
  });
  ...
  describe('exists', () => {
    ...
    it('should handle Redis errors', async () => {
      vi.mocked(redisClient.exists).mockRejectedValue(new Error('Redis error'));
  ...
  describe('getTTL', () => {
    ...
    it('should handle Redis errors', async () => {
      vi.mocked(redisClient.ttl).mockRejectedValue(new Error('Redis error'));
  ...
  describe('invalidateByTag', () => {
    it('should invalidate cache by tag', async () => {
      vi.mocked(redisClient.keys).mockResolvedValue(['test:tag:user:1', 'test:tag:user:2']);
      vi.mocked(redisClient.del).mockResolvedValue(2);
  ...
  describe('flush', () => {
    ...
    it('should throw CacheError on Redis error', async () => {
      vi.mocked(redisClient.flushDb).mockRejectedValue(new Error('Redis error'));
...
describe('CacheKeys', () => {
  ...
  it('should generate appointments cache key', () => {
    expect(CacheKeys.appointments('user-123', 2)).toBe('appointments:user:user-123:page:2');
  });
