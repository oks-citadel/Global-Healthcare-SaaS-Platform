...
describe.skip("DocumentService", () => {
  ...
  describe("uploadDocument", () => {
    ...
    it("should throw BadRequestError for invalid MIME type", async () => {
      ...
      await expect(
        documentService.uploadDocument(input, "provider-123", 1024),
    ...
    it("should throw BadRequestError for file too large", async () => {
      ...
      await expect(
        documentService.uploadDocument(input, "provider-123", largeFileSize),
      ).rejects.toThrow(BadRequestError);
    });
  ...
  describe("getDocumentById", () => {
    ...
    it("should throw NotFoundError for non-existent document", async () => {
      await expect(
        documentService.getDocumentById("non-existent-id"),
  ...
  describe("deleteDocument", () => {
    it("should delete a document by uploader", async () => {
      ...
      await expect(
        documentService.getDocumentById(uploaded.id),
      ).rejects.toThrow(NotFoundError);
    });
    ...
    it("should throw ForbiddenError when non-uploader tries to delete", async () => {
      ...
      await expect(
        documentService.deleteDocument(uploaded.id, "different-user"),
      ).rejects.toThrow(ForbiddenError);
    });

    it("should throw NotFoundError for non-existent document", async () => {
      await expect(
        documentService.deleteDocument("non-existent-id", "provider-123"),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe("generateDownloadUrl", () => {
    ...
    it("should throw NotFoundError for non-existent document", async () => {
      await expect(
        documentService.generateDownloadUrl("non-existent-id"),
      ).rejects.toThrow(NotFoundError);
    });
  });
