...
describe('EncounterService', () => {
  ...
  describe('createEncounter', () => {
    it('should create a new encounter successfully', async () => {
      ...
      const result = await encounterService.createEncounter(input, input.providerId);
  ...
  describe('getEncounterById', () => {
    it('should return encounter by ID', async () => {
      ...
      const created = await encounterService.createEncounter(input, input.providerId);
    ...
    it('should throw NotFoundError for non-existent encounter', async () => {
      await expect(encounterService.getEncounterById('non-existent-id'))
        .rejects
  ...
  describe('updateEncounter', () => {
    ...
    it('should update encounter status from in_progress to finished', async () => {
      ...
      await encounterService.updateEncounter(created.id, { status: 'in_progress' });
    ...
    it('should throw BadRequestError for invalid status transition', async () => {
      ...
      await expect(encounterService.updateEncounter(created.id, { status: 'finished' }))
        .rejects
        .toThrow(BadRequestError);
    });

    it('should throw NotFoundError for non-existent encounter', async () => {
      await expect(encounterService.updateEncounter('non-existent-id', { status: 'in_progress' }))
        .rejects
        .toThrow(NotFoundError);
    });
  });
  ...
  describe('addClinicalNote', () => {
    ...
    it('should throw NotFoundError for non-existent encounter', async () => {
      await expect(encounterService.addClinicalNote(
        'non-existent-id',
    ...
    it('should throw BadRequestError when adding note to cancelled encounter', async () => {
      ...
      const created = await encounterService.createEncounter(input, input.providerId);
      await encounterService.updateEncounter(created.id, { status: 'cancelled' });
