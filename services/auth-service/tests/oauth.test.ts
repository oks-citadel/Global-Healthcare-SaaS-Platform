...
describe('OAuth Service', () => {
  ...
  describe('getEnabledProviders', () => {
    it('should return list of enabled providers', async () => {
      const { getEnabledProviders } = await import('../src/strategies/index.js');
  ...
  describe('Google Strategy', () => {
    it('should generate correct authorization URL', async () => {
      const { googleStrategy } = await import('../src/strategies/google.strategy.js');
  ...
  describe('Facebook Strategy', () => {
    it('should generate correct authorization URL', async () => {
      const { facebookStrategy } = await import('../src/strategies/facebook.strategy.js');
      ...
      expect(url).toContain('scope=email%20public_profile');
    });
  });
});
...
describe('OAuth DTOs', () => {
  ...
  describe('UnlinkSocialAccountSchema', () => {
    it('should validate unlink input', async () => {
      const { UnlinkSocialAccountSchema } = await import('../src/dtos/oauth.dto.js');
...
describe('OAuth Error Codes', () => {
  it('should have all required error codes', async () => {
    ...
    expect(OAuthErrorCode.ACCOUNT_ALREADY_LINKED).toBe('ACCOUNT_ALREADY_LINKED');
    expect(OAuthErrorCode.ACCOUNT_NOT_LINKED).toBe('ACCOUNT_NOT_LINKED');
    expect(OAuthErrorCode.CANNOT_UNLINK_ONLY_AUTH).toBe('CANNOT_UNLINK_ONLY_AUTH');
  });
});
