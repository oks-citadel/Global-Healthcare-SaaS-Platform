...
- **Runtime Upgrade**: Upgraded from Node.js 20 LTS to Node.js 22 LTS across entire platform
- **Dockerfiles**: Updated all service and application Dockerfiles to use `node:22-alpine` or `node:22-slim`
- **CI/CD Pipelines**: Updated GitHub Actions workflows to use Node.js 22
- **Package Requirements**: Updated engine requirements in package.json files to `>=22.0.0`

##### Updated Dockerfiles

- `docker/Dockerfile.service` - Monorepo service Dockerfile
- `docker/Dockerfile.app` - Monorepo app Dockerfile
- `Dockerfile.web.pnpm` - Web frontend Dockerfile
- `packages/asset-branding/Dockerfile` - Asset branding Dockerfile
- All service Dockerfiles in `services/*/Dockerfile`:
  - api, api-gateway, auth-service
  - chronic-care-service, clinical-trials-service
  - denial-management-service, home-health-service
  - imaging-service, interoperability-service
  - laboratory-service, mental-health-service
  - notification-service, pharmacy-service
  - population-health-service, price-transparency-service
  - telehealth-service, vendor-risk-service

##### Updated CI/CD Configurations

- `.github/workflows/unified-pipeline.yml` - Main CI/CD pipeline
- `.github/workflows/asset-branding.yml` - Asset branding pipeline
- `.github/workflows/mobile-eas-pipeline.yml` - Mobile EAS build pipeline

##### Node.js 22 LTS Features Available

- Stable `fetch` API (no experimental flag needed)
- Stable WebSocket client
- Enhanced ES modules support
- Performance improvements in V8 engine
- Improved test runner stability
- Better TypeScript type stripping support (experimental)

##### Breaking Changes and Considerations

- Minimum Node.js version is now 22.0.0
- Some npm packages may need updates for Node.js 22 compatibility
- Native addons compiled for Node.js 20 will need recompilation
- `--experimental-require-module` flag now stable for ESM/CJS interop
