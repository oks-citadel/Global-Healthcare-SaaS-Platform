...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/register \
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/refresh \
  -H "Content-Type: application/json" \
  -d '{
    "refreshToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
  }'
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/api-keys \
...
```bash
curl -X GET https://api.unifiedhealthcare.com/api/v1/patients \
...
Redirect users to:
```
https://api.unifiedhealthcare.com/oauth/authorize?
  response_type=code&
  client_id={client_id}&
...
```bash
curl -X POST https://api.unifiedhealthcare.com/oauth/token \
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/mfa/enable \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
    "method": "totp"
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/mfa/verify \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "usr_abc123",
    "code": "123456"
...
```bash
curl -X GET https://api.unifiedhealthcare.com/api/v1/auth/permissions \
  -H "Authorization: Bearer {access_token}"
```
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/password/reset-request \
  -H "Content-Type: application/json" \
  -d '{
    "email": "patient@example.com"
  }'
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/password/reset \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PUT https://api.unifiedhealthcare.com/api/v1/auth/password/change \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/logout \
  -H "Authorization: Bearer {access_token}"
```
...
```bash
curl -X POST https://api.unifiedhealthcare.com/api/v1/auth/logout-all \
  -H "Authorization: Bearer {access_token}"
```
...
```bash
curl -X GET https://api.unifiedhealthcare.com/api/v1/auth/sessions \
  -H "Authorization: Bearer {access_token}"
```
