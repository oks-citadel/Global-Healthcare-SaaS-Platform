# PROMOTION OF ACCESS TO INFORMATION ACT (PAIA) MANUAL
## PAIA Act No. 2 of 2000 - Section 14

**Organization:** UnifiedHealth Platform, Inc.

**Information Officer:** [NAME]
**Email:** information.officer@unifiedhealth.com
**Phone:** [PHONE]
**Address:** [SA ADDRESS]

---

## 1. INTRODUCTION

This manual is prepared in accordance with Section 14 of the Promotion of Access to Information Act, 2000 (Act No. 2 of 2000) ("PAIA") and the Protection of Personal Information Act, 2013 (Act No. 4 of 2013) ("POPIA").

### 1.1 Purpose
This manual facilitates access to records held by UnifiedHealth Platform and describes the process for requesting access to such records.

### 1.2 Legislative Framework
- PAIA (Act No. 2 of 2000)
- POPIA (Act No. 4 of 2013)
- Constitution of South Africa, 1996

---

## 2. CONTACT DETAILS

### 2.1 Information Officer
**Name:** [INFORMATION OFFICER NAME]
**Email:** information.officer@unifiedhealth.com
**Phone:** [PHONE]
**Address:** [PHYSICAL ADDRESS]
**Postal Address:** [POSTAL ADDRESS]

### 2.2 Deputy Information Officer
**Name:** [DEPUTY NAME]
**Email:** deputy.io@unifiedhealth.com
**Phone:** [PHONE]

---

## 3. PAIA GUIDE

The South African Human Rights Commission (SAHRC) has compiled a guide on how to use PAIA, available at:

**Website:** www.sahrc.org.za
**Email:** PAIA@sahrc.org.za
**Phone:** +27 11 877 3600
**Address:** PAIA Unit, South African Human Rights Commission, Research and Documentation Department, Private Bag 2700, Houghton, 2041

---

## 4. RECORDS HELD

### 4.1 Categories of Records

**Personal Information Records:**
- Patient demographic information
- Identity documents
- Contact details
- Healthcare records
- Billing and payment information
- Consent records

**Healthcare Records:**
- Medical history
- Treatment records
- Prescriptions
- Laboratory results
- Imaging records
- Telemedicine consultation records

**Financial Records:**
- Invoices and payment records
- Insurance claims
- Transaction histories
- Billing statements

**Operational Records:**
- System access logs
- Audit trails
- Security incident reports
- Compliance documentation

**Employment Records:**
- Employee personal files
- Employment contracts
- Performance records
- Training records

**Legal and Compliance Records:**
- Contracts and agreements
- POPIA compliance documentation
- Policies and procedures
- Incident reports

### 4.2 Records Available Without Request

The following records are publicly available:
- PAIA Manual (this document)
- Privacy Policy
- Terms of Service
- General company information

Available at: www.unifiedhealth.com/legal

---

## 5. REQUESTING ACCESS TO RECORDS

### 5.1 Who Can Request Access

**Personal Requesters:**
- Data subjects requesting their own personal information
- Authorized representatives with written authority

**Other Requesters:**
- Any person requiring records for the exercise or protection of rights

### 5.2 Request Process

**Step 1: Complete Request Form**
- Use Form A (request by personal requester) or Form B (request by other requester)
- Forms available at www.unifiedhealth.com/paia or from our offices

**Step 2: Submit Request**
Submit to:
- **Email:** paia.requests@unifiedhealth.com
- **Post:** Information Officer, [POSTAL ADDRESS]
- **Physical Delivery:** [PHYSICAL ADDRESS]

**Step 3: Pay Prescribed Fee** (if applicable)
- Request fee: R50.00
- Access fees: As per PAIA regulations
- Payment details provided upon request

**Step 4: Await Response**
- Acknowledgment within 7 days
- Decision within 30 days of receipt

### 5.3 Required Information

Your request must include:
- Full name and contact details
- Identity or registration number
- Description of records requested
- Form in which access is required
- Postal or street address, fax number, or email
- Right being exercised or protected (if applicable)
- Proof of identity
- Proof of authority (if acting on behalf of another)

### 5.4 Fees

**Request Fee:** R50.00 (except personal requesters)

**Access Fees** (per PAIA regulations):
- Copies: R1.10 per page
- Printed copies: As per actual cost
- Computer-readable format: R75.00 + R7.50 per page
- Transcription: R24.00 per hour or part thereof
- Copy of visual images: Actual cost
- Search and preparation: R30.00 per hour (max 6 hours)

### 5.5 Response Times

- **Acknowledgment:** Within 7 days
- **Decision:** Within 30 days
- **Extension:** Additional 30 days (with notice and reasons)

---

## 6. GROUNDS FOR REFUSAL

Access may be refused if:
- Records do not exist or cannot be found
- Request is manifestly frivolous or vexatious
- Substantial and unreasonable diversion of resources
- Disclosure would involve unreasonable breach of privacy
- Commercial information would be disclosed
- Records are legally privileged
- Research information would be compromised
- Third party information without consent
- Safety or health of individuals would be endangered

---

## 7. PERSONAL INFORMATION PROTECTION

UnifiedHealth Platform processes personal information in accordance with POPIA:

### 7.1 Processing Purposes
- Healthcare service delivery
- Patient management
- Billing and payment processing
- Legal and regulatory compliance
- Quality improvement
- Research (with consent and anonymization)

### 7.2 Legal Basis
- Consent
- Contractual necessity
- Legal obligation
- Legitimate interests
- Public health

### 7.3 Data Subject Rights

You have the right to:
- **Access:** Request copies of your personal information
- **Correction:** Request correction of inaccurate information
- **Deletion:** Request deletion (subject to legal obligations)
- **Objection:** Object to processing for direct marketing
- **Restriction:** Request restriction of processing
- **Portability:** Receive information in machine-readable format

### 7.4 Exercising Rights

Contact the Information Officer:
- **Email:** information.officer@unifiedhealth.com
- **Response Time:** 30 days
- **No Fee:** For personal information requests

---

## 8. DATA SECURITY

UnifiedHealth Platform implements:

**Technical Measures:**
- Encryption (AES-256)
- Access controls
- Audit logging
- Network security
- Backup and disaster recovery

**Organizational Measures:**
- Staff training
- Confidentiality agreements
- Access management
- Incident response procedures
- Regular security assessments

**Data Residency:**
- All data stored in South African datacenters
- No transborder data flows without consent
- Compliance with POPIA Section 72

---

## 9. DATA BREACH NOTIFICATION

In the event of a data breach involving personal information:
- Information Regulator notified as soon as reasonably possible
- Affected data subjects notified without undue delay
- Details provided: nature of breach, likely consequences, measures taken
- Assessment of risk to data subjects

**Report Breaches:**
- **Email:** security@unifiedhealth.com
- **Phone:** [EMERGENCY NUMBER]

---

## 10. COMPLAINTS

### 10.1 Internal Complaints

Contact the Information Officer:
- **Email:** information.officer@unifiedhealth.com
- **Phone:** [PHONE]
- **Response:** Within 14 days

### 10.2 Information Regulator

If dissatisfied with response:

**Information Regulator (South Africa)**
- **Website:** www.justice.gov.za/inforeg
- **Email:** inforeg@justice.gov.za
- **Phone:** +27 10 023 5200
- **Address:** JD House, 27 Stiemens Street, Braamfontein, Johannesburg, 2001
- **Postal:** P.O Box 31533, Braamfontein, Johannesburg, 2017

### 10.3 South African Human Rights Commission

For PAIA matters:
- **Website:** www.sahrc.org.za
- **Email:** PAIA@sahrc.org.za
- **Phone:** +27 11 877 3600

---

## 11. UPDATES TO THIS MANUAL

This manual is reviewed annually and updated as necessary.

**Current Version:** 1.0
**Last Updated:** [DATE]
**Next Review:** [DATE]

Updates available at: www.unifiedhealth.com/paia

---

## 12. CONSENT AND AUTHORIZATION

### 12.1 Consent for Processing

By using UnifiedHealth Platform services, you consent to:
- Processing of personal information as described
- Storage of information in South African datacenters
- Disclosure to healthcare providers (with consent)
- Use for service improvement (anonymized)

### 12.2 Withdrawal of Consent

You may withdraw consent at any time:
- **Email:** privacy@unifiedhealth.com
- **Effect:** May limit service availability
- **Response:** Within 5 business days

---

## 13. THIRD PARTIES

### 13.1 Data Sharing

Personal information may be shared with:
- Healthcare providers (with consent)
- Medical aid schemes (for claims)
- Payment processors (for billing)
- Cloud service providers (for hosting)
- Regulators (legal requirement)

### 13.2 Operator Requirements

All third parties (operators) must:
- Sign Data Processing Agreements
- Comply with POPIA
- Implement security measures
- Limit processing to instructions
- Not transfer data outside SA without consent

---

## APPENDIX A: REQUEST FORMS

**Form A:** Request for Access to Personal Information (available at www.unifiedhealth.com/paia/form-a)

**Form B:** Request for Access to Records (other requesters) (available at www.unifiedhealth.com/paia/form-b)

**Form C:** Consent Form for Third-Party Requests (available at www.unifiedhealth.com/paia/form-c)

---

## APPENDIX B: FEE STRUCTURE

As prescribed in Government Gazette No. 45057, Notice R.1003

Details available in full manual at www.unifiedhealth.com/paia

---

## APPENDIX C: CONTACT DIRECTORY

**Information Officer:** information.officer@unifiedhealth.com
**Privacy Queries:** privacy@unifiedhealth.com
**PAIA Requests:** paia.requests@unifiedhealth.com
**Security Incidents:** security@unifiedhealth.com
**General Queries:** support@unifiedhealth.com

---

*This PAIA Manual has been prepared in compliance with Section 14 of the Promotion of Access to Information Act, 2000 (Act No. 2 of 2000) and the Protection of Personal Information Act, 2013 (Act No. 4 of 2013).*

**Approved by:**

Signature: ________________________________
Name: ____________________________________
Title: Information Officer
Date: _____________________________________
